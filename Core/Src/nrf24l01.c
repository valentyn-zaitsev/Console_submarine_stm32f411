...
#define nrf_csn(level) {HAL_GPIO_WritePin(NRF_CSN_GPIO_Port, NRF_CSN_Pin, (GPIO_PinState)level); delay_us(5);}

#define nrf_ce(level) HAL_GPIO_WritePin(NRF_CE_GPIO_Port, NRF_CE_Pin, (GPIO_PinState)level);
...
static uint8_t nrf_read_register(uint8_t reg)
{
	...
	nrf_csn(LOW);
	HAL_SPI_TransmitReceive(nrf_spi, &addr, &dt, 1, 1000);
	HAL_SPI_TransmitReceive(nrf_spi, (uint8_t*)0xff, &dt, 1, 1000);
	nrf_csn(HIGH);
	return dt;
}

static uint8_t nrf_write_registerMy(uint8_t reg, const uint8_t* buf, uint8_t len)
{
	uint8_t status = 0;
	uint8_t addr = W_REGISTER | (REGISTER_MASK & reg);

	nrf_csn(LOW);
	HAL_SPI_TransmitReceive(nrf_spi, &addr, &status, 1, 100);
	HAL_SPI_Transmit(nrf_spi, (uint8_t*)buf, len, 100);
	nrf_csn(HIGH);
	return status;
}

static uint8_t nrf_write_register(uint8_t reg, uint8_t value)
{
	uint8_t status = 0;
	uint8_t addr = W_REGISTER | (REGISTER_MASK & reg);
	nrf_csn(LOW);
...
uint8_t nrf_write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
	...
	nrf_csn(LOW);
	HAL_SPI_TransmitReceive(nrf_spi, &addr, &status, 1, 100);
	HAL_SPI_Transmit(nrf_spi, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		HAL_SPI_Transmit(nrf_spi, &empt, 1, 100);
	}
...
uint8_t nrf_read_payload(void* buf, uint8_t data_len)
{
	...
	nrf_csn(LOW);
	HAL_SPI_Transmit(nrf_spi, &addr, 1, 100);
	HAL_SPI_Receive(nrf_spi, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		HAL_SPI_Receive(nrf_spi, &empt, 1, 100);
	}
...
nrf_state nrf_init(SPI_HandleTypeDef *spi, rf24_pa_dbm_e level, rf24_datarate_e datarate, uint8_t channel, UART_HandleTypeDef *uart)
{
	...
	if (!nrf_isChipConnected()){
		//return NRF_INIT_ERROR;
	}
	...
	nrf_setRetries(5, 15);
	nrf_setPALevel(level); // Reset value is MAX
	...
	nrf_setDataRate(datarate); // Then set the data rate to the slowest (and most reliable) speed supported by all hardware.
	...
	HAL_UART_Transmit(debug_uart, (uint8_t*)str, strlen(str), 1000);
	...
	if(status == 0x00)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_PA_MIN\n", strlen("RF24_PA_MIN\n"), 1000);
	}
	else if(status == 0x01)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_PA_LOW\n", strlen("RF24_PA_LOW\n"), 1000);
	}
	else if(status == 0x02)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_PA_HIGH\n", strlen("RF24_PA_HIGH\n"), 1000);
	}
	else if(status == 0x03)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_PA_MAX\n", strlen("RF24_PA_MAX\n"), 1000);
	}
	...
	if(status == 0x02)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_250KBPS\n", strlen("RF24_250KBPS\n"), 1000);
	}
	else if(status == 0x01)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_2MBPS\n", strlen("RF24_2MBPS\n"), 1000);
	}
	else
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_1MBPS\n", strlen("RF24_1MBPS\n"), 1000);
	}
	...
	if(status == 0x00)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_CRC_DISABLED\n", strlen("RF24_CRC_DISABLED\n"), 1000);
	}
	else if(status == 0x01)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_CRC_8\n", strlen("RF24_CRC_8\n"), 1000);
	}
	else if(status == 0x02)
	{
	  HAL_UART_Transmit(debug_uart, (uint8_t*)"RF24_CRC_16\n", strlen("RF24_CRC_16\n"), 1000);
	}
#endif
	nrf_maskIRQ(true, true, true);

	return NRF_OK;
}

bool nrf_isChipConnected(void)
{
...
{
	nrf_ce(LOW); // Guarantee CE is low on powerDown
...
uint8_t nrf_getDynamicPayloadSize(void)
{
	...
	nrf_csn(LOW);
	addr = R_RX_PL_WID;
	HAL_SPI_TransmitReceive(nrf_spi, &addr, &result, 1, 1000);
	HAL_SPI_TransmitReceive(nrf_spi, (uint8_t*)0xff, &result, 1, 1000);
	nrf_csn(HIGH);
...
void nrf_toggle_features(void)
{
	...
	nrf_csn(LOW);
	HAL_SPI_Transmit(nrf_spi, &addr, 1, 1000);
	HAL_SPI_Transmit(nrf_spi, (uint8_t*)0x73, 1, 1000);
	nrf_csn(HIGH);
}
...
void nrf_writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
	...
	nrf_csn(LOW);
	HAL_SPI_Transmit(nrf_spi, &addr, 1, 1000);
	HAL_SPI_Transmit(nrf_spi, (uint8_t*)current, data_len, 1000);
	nrf_csn(HIGH);
}
